...
function computeStatus(now: Date, startDate: Date, endDate: Date): 'draft' | 'published' | 'ongoing' | 'completed' {
...
export const POST: RequestHandler = async (event) => {
  ...
  try {
    ...
    for (const a of rows) {
      ...
      const newStatus = computeStatus(now, new Date(a.start_date as any), new Date(a.end_date as any));
      if (newStatus !== a.status) {
        await db.update(activities).set({ status: newStatus, updatedAt: new Date() }).where(eq(activities.id, a.id));
        updated++;
      }
    }
