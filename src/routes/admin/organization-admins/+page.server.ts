...
import { AdminLevel, type OrganizationAdminDashboardStats, type ExtendedAdminRole, type Organization } from '$lib/types/admin';
  ...
  try {
    ...
    const rows = await db
      ...
      .where(and(
        ...
        or(eq(adminRoles.adminLevel, 'organization_admin'), eq(adminRoles.adminLevel, 'regular_admin')),
        eq(adminRoles.isEnabled, true),
        // OrganizationAdmin sees only their organization
        adminLevel === AdminLevel.OrganizationAdmin && (user.admin_role as any)?.organization_id ? eq(adminRoles.organizationId, (user.admin_role as any).organization_id) : (true as any)
      ))
    ...
    try {
      const org = await db
        .select({ id: organizations.id, name: organizations.name, code: organizations.code, status: organizations.status, created_at: organizations.createdAt, updated_at: organizations.updatedAt })
        .from(organizations)
    ...
    const stats: OrganizationAdminDashboardStats = {
      ...
      department_admins: organizationAdmins.filter(a => (a.assigned_departments?.length || 0) > 0).length,
    ...
    return {
      ...
      currentOrganization: adminLevel === AdminLevel.OrganizationAdmin ? organizationsList.find(f => f.id === ((user.admin_role as any)?.organization_id || '')) || null : null,
      organizations: adminLevel === AdminLevel.SuperAdmin ? organizationsList : organizationsList.filter(f => f.id === ((user.admin_role as any)?.organization_id || '')),
      stats,
      organizationAdmins,
      form
    };
