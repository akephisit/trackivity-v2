...
import { db, activities, organizations, users as dbUsers, departments, participations } from '$lib/server/db';
...
export const load: PageServerLoad = async (event) => {
	...
	try {
        ...
        const userParticipations = activityIds.length > 0 ? await db
            .select({
        ...
        const list = rows.map((r) => {
            ...
            const isEligible = !eligibleOrgs.length || // If no restrictions, everyone can join
