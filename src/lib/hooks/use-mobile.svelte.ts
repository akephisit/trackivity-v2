...
function detectDeviceInfo(): DeviceInfo {
	...
	// Device type detection
	const isMobile =
		width < BREAKPOINTS.md || /android|iphone|ipod|blackberry|iemobile|opera mini/i.test(userAgent);
	const isTablet =
		!isMobile && (width < BREAKPOINTS.lg || /ipad|tablet|kindle|silk/i.test(userAgent));
...
export function isLandscape(): boolean {
	let landscape = false;
	const unsubscribe = deviceInfo.subscribe(
		(info) => (landscape = info.orientation === 'landscape')
	);
	unsubscribe();
	return landscape;
}
