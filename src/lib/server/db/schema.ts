...
export const activityType = pgEnum('activity_type', ['Academic', 'Sports', 'Cultural', 'Social', 'Other']);
export const notificationType = pgEnum('notification_type', ['subscription_expiry', 'system_alert', 'admin_notice', 'faculty_update']);
...
export const organizationAnalytics = pgTable('organization_analytics', {
  ...
  averageParticipationRate: decimal('average_participation_rate', { precision: 5, scale: 2 }).default('0.00'),
...
export const subscriptionNotifications = pgTable('subscription_notifications', {
  ...
  return {
    subscriptionIdIdx: index('idx_subscription_notifications_subscription_id').on(table.subscriptionId),
    ...
    daysUntilExpiryIdx: index('idx_subscription_notifications_days_until_expiry').on(table.daysUntilExpiry),
  };
});
...
export const subscriptionExpiryLog = pgTable('subscription_expiry_log', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  subscriptionId: uuid('subscription_id').notNull().references(() => subscriptions.id, { onDelete: 'cascade' }),
  userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  ...
  return {
    subscriptionIdIdx: index('idx_subscription_expiry_log_subscription_id').on(table.subscriptionId),
    checkTimestampIdx: index('idx_subscription_expiry_log_check_timestamp').on(table.checkTimestamp),
  };
});
