...
async function main() {
  ...
  console.log(`Latency (ms): p50=${p50.toFixed(1)} p90=${p90.toFixed(1)} p95=${p95.toFixed(1)} p99=${p99.toFixed(1)} avg=${avg.toFixed(1)} min=${min.toFixed(1)} max=${max.toFixed(1)}`);
  ...
  console.log(`SLO check (p95<300ms & fail<1%): ${meetsSLO ? "PASS" : "FAIL"} (fail=${failRate.toFixed(2)}%)`);
}
