...
const ROUTE_CONFIG = {
	...
	STUDENT_PROTECTED: ['/student', '/dashboard', '/profile', '/activities', '/qr', '/history'],

	// Admin protected routes - require admin authentication
	ADMIN_PROTECTED: ['/admin'],
...
function isPublicRoute(pathname: string): boolean {
	return ROUTE_CONFIG.PUBLIC.some((route) => {
		...
		return pathname === route || pathname.startsWith(route + (route.endsWith('/') ? '' : '/'));
	});
}
...
function isStudentProtected(pathname: string): boolean {
	return ROUTE_CONFIG.STUDENT_PROTECTED.some(
...
function isAdminProtected(pathname: string): boolean {
	...
	if (
		ROUTE_CONFIG.ADMIN_LOGIN.some((route) => pathname === route || pathname.startsWith(route + '/'))
	) {
		return false;
	}

	return ROUTE_CONFIG.ADMIN_PROTECTED.some(
		(route) => pathname === route || pathname.startsWith(route + '/')
	);
}
